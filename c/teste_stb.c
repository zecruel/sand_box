...
int main(int argc, char** argv){
	...
	bmp_img stb_img, *tool1 = NULL, *tool2 = NULL, *tool3 = NULL;
	stb_img.buf = NULL;
	...
	while (quit == 0){
		...
		if(action == FILE_OPEN) {
			...
			if (url){
				...
				//progress = 0;
				//low_proc = 0;
				stbi_image_free(stb_img.buf);
				
				int num_ch;
				unsigned char *data = stbi_load(url, &(stb_img.width), &(stb_img.height), &num_ch, 4);
				/*
				if (data){
					if (stb_img.width > 16){
						stb_img.width = 16;
					}
					if (stb_img.height > 16){
						stb_img.height = 16;
					}
				}*/
				...
				/*
				if (data) {
					void *new_img = malloc(100*100*4);
					stbir_resize_uint8(data , stb_img.width , stb_img.height , 0, new_img, 100, 100, 0, 4);
					free(data);
					data = new_img;
					stb_img.width = 100;
					stb_img.height = 100;
				...
				tool1 = bmp_sub_img(&stb_img, 0,0,16,16);
				tool2 = bmp_sub_img(&stb_img, 16,0,16,16);
				tool3 = bmp_sub_img(&stb_img, 32,0,16,16);
				draw=1;
			}
		...
		if ((gui_check_draw(gui) != 0) || (draw != 0)){
			...
			if (stb_img.buf){
				bmp_copy(&stb_img, img, 100, 100);
				bmp_copy(tool1, img, 200, 200);
				bmp_copy(tool2, img, 300, 200);
				bmp_copy(tool3, img, 400, 200);
			}
